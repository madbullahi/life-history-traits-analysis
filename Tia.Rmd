...
```{r}
MicroplasticOnly <- MP_Data[-c(1,5:6,8:9)]

```


```{r Subset of Microplastic Table}
...
```{r}
MicroplasticOnly <- MicroplasticOnly[complete.cases(MicroplasticOnly),]
...
```{r}
MicroplasticOnly[3:5]<- log(MicroplasticOnly[3:5]+0.5)

```


```{r}
logMicroplasticOnly_data <- MicroplasticOnly[,c(3:5)]

```

```{r}
logMicroplasticOnly_data

```


```{r}
logMicroplasticOnly_pca <- prcomp(logMicroplasticOnly_data, scale. = TRUE)

```

```{r}
logMicroplasticOnly_pca
```
...
summary(logMicroplasticOnly_pca)
```
...
fviz_pca_biplot(logMicroplasticOnly_pca,
             col.ind = Genotypes,
...
```{r}
fviz_pca_var(logMicroplasticOnly_pca, geom = c("point", "text"))
...
fviz_pca_biplot(logMicroplasticOnly_pca, label="var", habillage=MP_Data$Genotypes,
               addEllipses=TRUE, ellipse.level=0.95)
```

```{r}

fviz_pca_ind(logMicroplasticOnly_pca)
```
...
logMP_PF_PA <- MP_PF_PA[,c(3:5)]
```

```{r}

logMP_PF_PA_pca <- prcomp(logMP_PF_PA, scale. = TRUE)
...
Genotypes <- as.factor(MP_PFOS_PFOA$Genotypes)
...
fviz_pca_biplot(logMP_PF_PA_pca,
             col.ind = Genotypes,
              palette = c("LRV-0-1"="red","LR2-36-01"="black"),
             addEllipses = TRUE, 
             col.var="blue",
