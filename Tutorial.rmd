...
```{r}
Genotype <- as.factor(comp_D_WO$Genotype) # convert Genotype to factor

```

```{r}

Treatment <- as.factor(comp_D_WO$Treatment) # convert Treatment to factor
...
Pca_plot <- fviz_pca_ind(pca, habillage = Genotype, geom = "point") # fviz_pca_biplot() function from factoextra package is used to plot the scores for the first two PCs.
```
...
fviz_pca_biplot(pca,
             col.ind = Treatment, 
             palette = c("PET"="red","PC1"="black", "PC2" = "blue", "PC1C2" = "green"),
             addEllipses = TRUE, 
             col.var="blue",
             arrowsize=0.2,
             labelsize=3,
...
# Assuming Genotype and Treatment are columns in your dataset
# Ensure that Genotype and Treatment are factors
#log_comp_D_WO$Genotype <- factor(log_comp_D_WO$Genotype)
#log_comp_D_WO$Treatment <- factor(log_comp_D_WO$Treatment)

# Create the PCA biplot with color assignment for both Genotype and Treatment
fviz_pca_biplot(pca,
             habillage = comp_D_WO$Genotype, # Color by Treatment
            # col.ind = Genotype, 
             addEllipses = TRUE, 
             col.var="blue",
             arrowsize=0.2,
...
```{r}
qplot(U[, 1], U[, 2]) + coord_equal() # plot the first two PCs
```
