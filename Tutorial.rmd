...
#model_PET <- bglmer(Mortality ~ Treatment * Genotype + (1|Genotype:Replicates), data = PET_mortality, family #= binomial) # create the model
...
p <- length(levels(taxa)) # create the p variable
...
```{r}
###NOTE: summary statistics pnl;y valid for 2+ trajectories!!
sumstatsize.obs
PSumSize
sumstatdir.obs
PSumOrient
...
for(i in 1:n){
  points(traj.specs.obs[p,1,i],traj.specs.obs[p,2,i],pch=21,col="green", cex=1.5, bg="black")
}
```
