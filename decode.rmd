...
# Adjust the length of 'cleaned_data' to match the number of rows in 'cleaned_df'
if(length(cleaned_data) < nrow(cleaned_df)) {
  cleaned_data <- c(cleaned_data, rep(NA, nrow(cleaned_df) - length(cleaned_data)))
} else if(length(cleaned_data) > nrow(cleaned_df)) {
  cleaned_data <- cleaned_data[1:nrow(cleaned_df)]
}
...
write.csv(cleaned_df, "cleaned_data.csv")
